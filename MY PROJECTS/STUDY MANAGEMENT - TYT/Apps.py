...
class Timer():
    INIT_TIMER_HOLDER = None  # This is a class variable in which we will hold
    def __init__(self):
        pass
    ...
    class Countdown(Frame):
        ...
        def Countdown_timer(self, master, time, Track_card_bar=None):  # Toplevel app
            ...
            """
            Timer.INIT_TIMER_HOLDER.withdraw()
            ...
            self.img = tkinter.PhotoImage(file='./Resources/icons/icons8_up_left_16.png')
            lift_button = Button(master, text='-', image=self.img, cnf=b_cfg)
            lift_button.place(x=0, y=0)
            ...
            lift_button.config(command=chlift)
            ...
            def update_countdown_timer():
                ...
                if New_sec.time_to_seconds(list(updated_time)) >= 0:
                    ...
                    if self.Stopwatch is None:
                        ...
                        time_label.config(
                            text="+" + additional_time)  # updating the Extra time on the Countdown timer label.
            ...
            def save_data():
                ...
                :return:
                """
                Timer.INIT_TIMER_HOLDER.deiconify()
        ...
        def Ask_Countdown_Time(self, master, data=None, Track_card_bar=None):  # Top level app
            ...
            """
            Timer.INIT_TIMER_HOLDER.withdraw() # when Ask_Countdown_Timer will open our init timer app will disappear.
            ...
            hour_label = Label(time_frame, text="Hour", font=("serif", 40, 'bold'), bg=self.bg_color, fg=self.fg_color)
            min_label = Label(time_frame, text="Min", font=("serif", 40, 'bold'), bg=self.bg_color, fg=self.fg_color)
            sec_label = Label(time_frame, text="Sec", font=("serif", 40, 'bold'), bg=self.bg_color, fg=self.fg_color)
            ...
            hour_entry = Spinbox(time_frame, width=2, font=("serif", 40, 'bold'), bg=self.fg_color, fg=self.bg_color,
            ...
            # putting colon
            Label(time_frame, text=":", font=("serif", 40, 'bold'), bg=self.bg_color, fg=self.fg_color).grid(row=1,
                                                                                                             column=1)

            min_entry = Spinbox(time_frame, width=2, font=("serif", 40, 'bold'), bg=self.fg_color, fg=self.bg_color,
            ...
            sec_entry = Spinbox(time_frame, width=2, font=("serif", 40, 'bold'), bg=self.fg_color, fg=self.bg_color,
                                buttonbackground=self.fg_color, buttondownrelief=FLAT, relief=FLAT, buttonup=FLAT,
                                from_=00, to=60, format='%02.0f')
            ...
            start_timer_button = Button(time_frame, text='Start', font=("serif", 15, 'bold'), bg=self.bg_color,
                                        fg=self.fg_color
                                        , relief=FLAT, activebackground=self.bg_color, activeforeground=self.fg_color,
                                        bd=0)
            start_timer_button.grid(row=2, column=1, columnspan=3, pady=10)

            close_button = Button(time_frame, text='x', font=("serif", 8, 'bold'), bg=self.bg_color, fg=self.fg_color
            ...
            def onclose():
                # configuration method for closing the Ask_Countdown_Timer app
                Timer.INIT_TIMER_HOLDER.deiconify()
                master.destroy()
            close_button.config(command=onclose)
            ...
            def set_timer():
                ...
                self.Countdown_timer(root, time,
                                     Track_card_bar=Track_card_bar)  # Here Now we will start the Countdown timer.
                root.mainloop()
        ...
        def init_Timer(self, Master):
            ...
            Outer_Container_Frame.pack(fill='both', expand=1)

            Activities = Widgets.Widgets.Activities(Outer_Container_Frame, timer_class=Timer, width=500,
                                                    bg=self.bg_color)
            ...
            Activities.theme_configure(bar_bg_color=self.bg_color, bar_item_bg=self.bg_color, bar_item_fg=self.fg_color)
            ...
            timer_frame = Frame(Outer_Container_Frame, width=400, height=50, bg=self.bg_color, relief=GROOVE)
            ...
            # timer_label = Label(timer_frame, text="Start Timer", font=("sarif", 35, 'bold'), bg=self.bg_color,
            #                     fg=self.fg_color)
            # timer_label.pack(pady = (0,32), fill='y', expand=1)
            ...
            #                             command=lambda x=timer_frame, y=timer_label: self.continue_timer(x, y))
            #
            # # Now when i press the start_timer_button then a new top level window should appear.
            # start_timer_button.place(x=110, y=60)
        ...
        def Display_Start(self, master):  # Top level app
            ...
            Start_countdown_timer_button = Button(Start_Frame, text='Start', relief=GROOVE, bg=self.bg_color,
                                                  fg=self.fg_color,
                                                  highlightbackground=self.bg_color, highlightcolor='white',
...
class Tracks(Frame):
                ...
                if l2l == l1h and l2h > l2l:
                    if l2h == l3l and l3h > l3l:
                        ...
                        self.target_datadict = {"Date": ttime.strftime("%d-%b-%Y"),
                                                ...
                                                "l2l": l2l, "l2h": l2h,
                                                "l3l": l3l, "l3h": l3h,
                                                "Total Time": tth.get_today_target()['Total Time']}

                        save = apps.SaveTime()
                        save.save_today_target_time(self.target_datadict)
                        self.set_today_target_hour = ((l1l, l1h), (l2l, l2h), (l3l, l3h))
                        master.destroy()
                        Widgets.Widgets.TTH_ProgressBar.TTH_BAR_RESET = True
                        # giving order to tth bar the we are resetting so you need to retrieve data again.
                    else:
    ...
    def Add_Time_Record(self, master=None):
        ...
        """
        style = ttk.Style()
        self.init_timer_after_id = None
        act = apps.Activity_Classes()
        database = act.retrieve_activities()
        root_activities = [key for key in database["ActivityClasses"].keys()]
        self.act_list2 = None
        self.act_list3 = None
        self.act_list4 = None

        act_var1 = StringVar()
        act_var1.set(root_activities[0])
        act_var2 = StringVar()
        act_var3 = StringVar()
        act_var4 = StringVar()
        ...
        if master.winfo_class() == 'Tk' or master.winfo_class() == 'Toplevel':
            ...
            master.config(bg='#0389a3')
        self.Add_Time_Record_master = master
        ...
        l_cfg = {'fg': 'white', 'bg':master.cget('bg')}
        style.configure("add.TCombobox", bordercolor='white', fieldbackground='white', foreground='black',
                        background='white', darkcolor='black', lightcolor='white')

        style.configure("Timer.TButton", background=master.cget('bg'), bordercolor='white', darkcolor='#0389a3', lightcolor='white',
                        foreground='white', font=('sarif',10,'bold'))
        cpadx = 25
        cpady = 10

        option_label = Label(master, text='Record Name', cnf=l_cfg)
        option_label.grid(row=0, column=0, pady=10,  sticky='w', padx=cpadx)

        options_menu = ttk.Combobox(master, textvariable=act_var1, values=root_activities, style="add.TCombobox")
        options_menu.grid(row=0, column=1, pady=(10, 5), padx=cpadx)

        time_record_name_label = Label(master, text='Record Name', cnf=l_cfg)
        time_record_name_label.grid(row=1, column=0, pady=cpady,  sticky='w', padx=cpadx)

        time_record_name_entry = ttk.Combobox(master, textvariable=act_var2, values=self.act_list2,
                                              style="add.TCombobox")
        time_record_name_entry.grid(row=1, column=1, pady=cpady)

        time_topic_label = Label(master, text='Topic', cnf=l_cfg)
        time_topic_label.grid(row=2, column=0, sticky='w', padx=cpadx)

        time_topic_entry = ttk.Combobox(master, textvariable=act_var3, values=self.act_list3, style="add.TCombobox")
        time_topic_entry.grid(row=2, column=1, pady=cpady)

        time_record_subtopic_label = Label(master, text='Sub Topic', cnf=l_cfg)
        time_record_subtopic_label.grid(row=3, column=0, pady=cpady,  sticky='w', padx=cpadx)

        time_record_subtopic_entry = ttk.Combobox(master, textvariable=act_var4, values=self.act_list4,
                                                  style="add.TCombobox")
        time_record_subtopic_entry.grid(row=3, column=1, pady=cpady)

        start_time_record_button = ttk.Button(master, text='Start', style="Timer.TButton")
        start_time_record_button.place(x=160, y=170)

        close_button = ttk.Button(master, text='X', style="Timer.TButton")
        close_button.place(x=335, y=0)
        ...
        def get_acts():
            """
            This method will be used to get act_var variable after selection one:
            :return:
            """
            # try:
            data = database["ActivityClasses"]
            act1 = act_var1.get()
            if isinstance(data[act1]['Activities'], dict) and bool(data[act1]['Activities']):
                self.act_list2 = [key for key in data[act1]['Activities'].keys()]
                time_record_name_entry.config(values=self.act_list2)
                data = data[act1]['Activities']
                act2 = act_var2.get()

                if isinstance(data.get(act2, None), dict):
                    if bool(data[act2]['Activities']):
                        self.act_list3 = [key for key in data[act2]['Activities'].keys()]
                        time_topic_entry.config(values=self.act_list3)
                        data = data[act2]['Activities']
                        act3 = act_var3.get()

                        if isinstance(data.get(act3, None), dict):
                            if bool(data[act3]["Activities"]):
                                self.act_list3 = [key for key in data[act2]['Activities'].keys()]
                                time_record_subtopic_entry.config(values=self.act_list3)

            self.init_timer_after_id = master.after(1, get_acts)
        ...
        def init_countdown_timer():
            ...
            master.after_cancel(self.init_timer_after_id)
            record_data = {"Record Type": act_var1.get(),
                           "Record Name": act_var2.get(),
                           "Record Topic": act_var3.get(),
                           "Record SubTopic": act_var4.get()}
            root = Toplevel()
            self.Add_Time_Record_master.destroy()
