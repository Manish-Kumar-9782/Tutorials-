...
<body class="p-5">
    ...
    <!-- 
        ...
        In this section We will learn about the Prepared Statement and their requirement.

        A prepared statement is a feature used to execute the same (or similar) SQL 
        statement repeatedly with high efficiency.

        The Prepared statement execution consists of two stages: Prepare adn Execute.

        Prepare: An SQL statement template is created and sent to the database server. 
            The server performs a syntax check and initialize server internal resources 
            for later use.

        Execute: At the Execute stage the parameter values are sent to the server.
            The server creates a statement from the statement template and these values 
            to execute it.
