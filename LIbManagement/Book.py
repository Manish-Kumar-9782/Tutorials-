...
class Book:
    ...
    def fInline_display(sequence, column_len=20, str_len=18):
        ...
        str_len = str_len if isinstance(
            str_len, (list, tuple)) else [str_len]*6
        column_len = column_len if isinstance(
            column_len, (list, tuple)) else [column_len]*6
    ...
    def edit(self):
        offset = 20
        print("EntryName".ljust(offset), "oldValue".ljust(
            offset), "newValue".ljust(offset))
...
class BookDatabase:
    ...
    def displayBooks(self):
        displaySep("=", 100)
        Book.fInline_display(self.reader.headers,
                             self.column_sizes, self.str_lengths)
        displaySep("=", 100)
        for book in self.books:
            Book.fInline_display(
                book.to_list(), self.column_sizes, self.str_lengths)
            displaySep("-", 100)
    ...
    def saveDatabase(self):
        ...
        self.writer.write_rows(
            list(map(lambda x: x.to_list(), self.books)), self.header)
